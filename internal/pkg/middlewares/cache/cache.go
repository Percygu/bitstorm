...
func (client *Client) Incr(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		log.Errorf("Redis Incr error: %v", err.Error())
		return "", err
	}
	return ret, nil
}
...
func (client *Client) Eval(ctx context.Context, script string, keys []string, args ...interface{}) (bool, error) {
	conn := redisConn.Conn()
	defer conn.Close()
...
func (client *Client) HSet(ctx context.Context, key, field, value string) (int64, error) {
	conn := redisConn.Conn()
	defer conn.Close()
