...
func (r *RedisLock) DelayExpire(ctx context.Context, expireSeconds int64) error {
	result, err := cache.GetRedisCli().Eval(ctx, LuaCheckAndExpireDistributionLock, []string{r.key}, []interface{}{r.token, expireSeconds})
	if err != nil {
		return nil
	}
...
func (r *RedisLock) Unlock(ctx context.Context) error {
	...
	result, err := cache.GetRedisCli().Eval(ctx, LuaCheckAndDeleteDistributionLock, []string{r.key}, r.token)
	if err != nil {
		return err
	}
